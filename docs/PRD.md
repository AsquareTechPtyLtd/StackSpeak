...
5. **Assessment Quiz** — multiple-choice quiz in Review tab. Each correct answer earns one assessment point toward level progression; a word earns at most two points, and the second correct must land on a different day than the first (shown as "1/2 correct"). A wrong answer triggers a short retry cooldown (not a full-day lockout). After the second correct, the app offers to mark the word mastered (out of daily rotation) — declining keeps it rotating; a simultaneous level-up takes precedence over the offer.
...
**Progression currency:** **assessment points**. Every correct assessment answer earns one point, and a word earns at most two — its first correct, plus a second correct that must land on a later day (same-day reattempts after a correct answer are blocked; see `UserProgress.assessmentPointsForLevel`, `canAttemptAssessment`). Thresholds in `Models/Level.swift` are denominated in points (two per word relative to the old word counts). Skip/report marks a word mastered but grants **no** points; "mastered" is not progression currency. Streak does not affect leveling.
