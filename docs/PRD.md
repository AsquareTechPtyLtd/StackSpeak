...
5. **Assessment Quiz** — multiple-choice quiz in Review tab. A word is credited toward level progression on its **second** correct answer, which must land on a different day than the first (one correct answer is in-progress, shown as "1/2 correct"). A wrong answer triggers a short retry cooldown (not a full-day lockout). After the second correct, the app offers to mark the word mastered (out of daily rotation) — declining keeps it rotating; a simultaneous level-up takes precedence over the offer.
...
**Progression currency:** a word is *credited* toward leveling on its **second correct assessment answer**, which lands on a later day than the first — same-day reattempts after a correct answer are blocked (`UserProgress.wordsAssessedForLevel`, `canAttemptAssessment`). The first correct answer is in-progress state ("1/2 correct"), not credit. Skip/report marks a word mastered but grants **no** level credit; "mastered" is not progression currency. Streak does not affect leveling.
